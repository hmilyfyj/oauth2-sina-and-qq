...
__version__ = '1.0.1'
__author__ = 'luo weihua'

'''
Python client SDK for OAuth 2.
'''
...
def _http_call(the_url, method, authorization, **kw):
    ...
    try:
        ...
        #socket error
        raise APIError("501", "Socket error", "")
...
class HttpObject(object):
    ...
    def __getattr__(self, attr):
        def wrap(**kw):
            url = self.client.api_url + attr.replace('__', '/') \
                    + ('.'+self.client.format if self.client.format else '' )
            kw.update(self.client.params)        
            r= _http_call(url, self.method, self.client.access_token, **kw)
            return self.client.parse_result(r)
        return wrap
...
class APIClient(object):
    ...
    '''
    def __init__(self, app_key, app_secret, type=None, redirect_uri=None, oauth_uri='', domain='', format=None, version=None):
        self.type = type
        self.client_id = str(app_key)
        self.client_secret = str(app_secret)
        self.redirect_uri = redirect_uri
        self.auth_url = 'https://%s/%s/' % (domain, oauth_uri)
        self.api_url = 'https://%s/%s/' % (domain, version) if version else 'https://%s/' % domain
        self.format = format
    ...
    def set_access_token(self, access_token, expires=0):
        self.access_token = str(access_token)
        self.expires = float(expires)

    def set_request_params(self, **kw):
        self.params = kw
        
    def get_authorize_url(self, redirect_uri=None, authorize_uri='authorize', **kw):
        '''
        ...
        return '%s%s?%s' % (self.auth_url, authorize_uri, \
                _encode_params(client_id = self.client_id, \
                        response_type = response_type, \
                        redirect_uri = redirect, **kw))
                        
    def request_openID(self, uri='me'):
        
        r = _http_get('%s%s' % (self.auth_url, uri), \
            access_token = self.access_token)
        return self.parse_result(r)
            
    def request_access_token(self, code, redirect_uri=None,
                             method='POST',  token_uri='', 
        ...
        r= _http_call('%s%s' % (self.auth_url, token_uri), method, None,
        ...
        r = self.parse_result(r)
        self.access_token = r.access_token
        current = int(time.time())
        r.expires_in = int(r.expires_in)
        r.expires = r.expires_in + current
        return r
...
#                expires = rtime
    ...
    def check_error(self, r):
        ...
        if self.type is 'qq':
            if hasattr(r, 'ret'):
                if r.ret is not 0:
                    raise APIError(r.ret, r.msg, '')
            elif hasattr(r, 'error'):
                raise APIError(r.error, r.error_description, '')
            
        elif self.type is 'sina':
            if hasattr(r, 'error'):
    ...
    def parse_result(self, r):
        '''
        qq result format is :
        access_token=A34C3A391DD9F0B751EDAD3998BC1DFB&expires_in=7776000&refresh_token=BE5844DAD062F139982FDB4551046C7E
        callback( {"error":100020,"error_description":"code is reused error"} );
        {"ret":-1,"msg":"client request's parameters are invalid"}
        '''
        ...
        try:
            ...
            import urlparse
            r = JsonDict(urlparse.parse_qsl(r))
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        ...
        if method==_HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        kw.update(self._client.params)     
        r = _http_call('%s%s.json' % (self._client.api_url, self._path), method, self._client.access_token, **kw)
        return self._client.parse_result(r)
